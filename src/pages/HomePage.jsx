// src/pages/HomePage.jsx
import React, { useState } from 'react';
import ProductCard from '../components/ProductCard'; 

const HomePage = ({ data }) => {
    // Destructuring data dari JSON
    const { heroSection, promoSection, products, footer } = data;
    
    // 1. State untuk filtering kategori di "Our Collection"
    const [activeCategory, setActiveCategory] = useState('All Products'); 
    
    // 2. Ambil 2 produk pertama sebagai "New Arrivals"
    const newArrivals = products.slice(0, 2); 

    // 3. Fungsi Filter Produk untuk Section "Our Collection" - Menggunakan properti category
    const filteredProducts = products.filter(product => {
        if (activeCategory === 'All Products') return true;
        
        // Mencocokkan nilai properti category
        if (activeCategory === product.category) return true;
        
        return false;
    });
    
    // Daftar kategori untuk tombol filter
    const categories = ['All Products', 'Pashmina', 'Jilbab Segi Empat'];


    return (
        <div className="home-page-container">
            {/* --------------------------------- */}
            {/* Section 1: HERO & VISION/MISSION (ID: about) */}
            {/* --------------------------------- */}
            <section id="about" className="section hero-section">
                <div className="hero-content">
                    <h2 className="hero-main-title">{heroSection.title}</h2>
                    <div className="vision-mission-container">
                        <div className="vision-box">
                            <h3 className="sub-title">Our Vision</h3>
                            <p>{heroSection.vision}</p>
                        </div>
                        <div className="mission-box">
                            <h3 className="sub-title">Our Mission</h3>
                            <p>{heroSection.mission}</p>
                        </div>
                    </div>
                </div>
                <div className="hero-image-col">
                    {/* Gambar di folder public/assets/images/ */}
                    <img src="/assets/images/model-hero.jpg" alt="Elegance Meets Modesty" className="hero-image" />
                </div>
            </section>

            {/* --------------------------------- */}
            {/* Section 2: PROMO & NEW ARRIVALS (ID: promo) */}
            {/* --------------------------------- */}
            <section id="promo" className="promo-new-arrivals-section">
                
                {/* Kolom Kiri: Promo Banner (Background Pink) */}
                <div className="promo-box">
                    <span className="promo-tag-label">Limited Time Offer</span>
                    <h2 className="promo-offer">{promoSection.offer}</h2>
                    <p className="promo-tagline">{promoSection.tagline}</p>
                    <p className="promo-description">{promoSection.description}</p>
                    <a href="#products" className="promo-btn">Shop Now &rarr;</a>
                </div>

                {/* Kolom Kanan: New Arrivals */}
                <div className="new-arrivals-container">
                    <h3 className="new-arrivals-title">New Arrivals</h3>
                    <p className="new-arrivals-subtitle">Discover our latest collection</p>
                    <div className="new-arrivals-grid">
                        {/* Merender 2 produk terbaru */}
                        {newArrivals.map(product => (
                            <ProductCard 
                                key={product.id} 
                                product={product} 
                                currency={data.general.currency} 
                            />
                        ))}
                    </div>
                </div>
            </section>

            {/* --------------------------------- */}
            {/* Section 3: OUR COLLECTION (ID: products) - Filtering Produk */}
            {/* --------------------------------- */}
            <section id="products" className="section products-collection-section">
                <h2 className="section-title">Our Collection</h2>
                
                {/* Filter Buttons */}
                <div className="product-filters">
                    {categories.map(cat => (
                        <button 
                            key={cat} 
                            onClick={() => setActiveCategory(cat)}
                            // Menambahkan class 'active' berdasarkan state
                            className={`filter-btn ${activeCategory === cat ? 'active' : ''}`}
                        >
                            {cat}
                        </button>
                    ))}
                </div>
                
                {/* Grid Produk yang sudah difilter */}
                <div className="product-grid">
                    {filteredProducts.map(product => (
                        <ProductCard 
                            key={product.id} 
                            product={product} 
                            currency={data.general.currency} 
                        />
                    ))}
                </div>
                {/* Tombol view more untuk navigasi ke halaman produk penuh (jika ada) */}
                <div className="view-more-container">
                    <a href="/products" className="view-more-link">View All Products</a>
                </div>
            </section>

            {/* --------------------------------- */}
            {/* Section 4: FOOTER (ID: contact-us) */}
            {/* --------------------------------- */}
            <footer id="contact-us" className="footer-section">
                <div className="footer-content">
                    <div className="footer-logo">{data.general.logo}</div>
                    <div className="footer-links">
                        <h4 className="footer-heading">Quick Links</h4>
                        <ul>
                            {footer.quickLinks.map(link => <li key={link}><a href="#top">{link}</a></li>)}
                        </ul>
                    </div>
                    <div className="footer-contact">
                        <h4 className="footer-heading">Contact Us</h4>
                        <p>📍 {footer.address}</p>
                        <p>📞 {footer.phone}</p>
                        <p>📧 {footer.email}</p>
                    </div>
                </div>
                <div className="footer-bottom">
                    &copy; {new Date().getFullYear()} {data.general.logo}. All rights reserved.
                </div>
            </footer>
        </div>
    );
};

export default HomePage;